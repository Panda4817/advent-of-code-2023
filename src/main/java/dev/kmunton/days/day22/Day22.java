...
public class Day22 implements Day {
    ...
    public void processData(List<String> input) {
        ...
        for (var line : input) {
            ...
            var end1 = new Point3D(end1Array[0], end1Array[1], end1Array[2]);
            var end2 = new Point3D(end2Array[0], end2Array[1], end2Array[2]);
    ...
    public long part1() {
        ...
        return bricksThatCanBeRemovedWithoutOtherBricksFalling;

    }

    private boolean isBrickAtRest(Brick brick, Integer id) {
        int minZ = brick.getAllCubes().stream().map(Point3D::getZ).min(Integer::compareTo).orElseThrow();
        if (minZ == 1) {
            return true;
        }
        for (var point : brick.getAllCubes()) {
            ...
            var pointBelow = new Point3D(point.getX(), point.getY(), point.getZ() - 1);
    ...
    private Brick updateBrickEnds(Brick brick, Integer id) {
        while (!isBrickAtRest(brick, id)) {
            ...
            var newEnd1 = new Point3D(end1.getX(), end1.getY(), end1.getZ() - 1);
            var newEnd2 = new Point3D(end2.getX(), end2.getY(), end2.getZ() - 1);
    ...
    private void populateSupportsMap() {
        for (var entry : bricks.entrySet()) {
            ...
            var highestZ = brick.getAllCubes().stream().map(Point3D::getZ).max(Integer::compareTo).orElseThrow();
            var pointsAboveBrick = brick.getAllCubes().stream()
                .filter(p -> p.getZ() == highestZ)
                .map(p -> new Point3D(p.getX(), p.getY(), p.getZ() + 1)).toList();
    ...
    private void makeBricksFall() {
        ...
        while(isProcessed.values().stream().anyMatch(v -> !v)) {
            ...
            for (var entry : bricks.entrySet()) {
                ...
                var minZ = Math.min(entry.getValue().getEnd1().getZ(), entry.getValue().getEnd2().getZ());
                if (minZ == currentZ) {
                    var brick = entry.getValue();
